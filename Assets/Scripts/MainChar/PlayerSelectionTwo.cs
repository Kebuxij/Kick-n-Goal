...
public class PlayerSelectionTwo : MonoBehaviour
{
    ...
    private static PlayerSelectionTwo selectedObject; // Ссылка на текущий выбранный объект
    ...
    void Start()
    {
        // Проверяем, находится ли объект в слое "SelectableTwo"
        if (gameObject.layer != LayerMask.NameToLayer("SelectableTwo"))
        {
            //Debug.LogWarning($"Объект {gameObject.name} не находится в слое 'SelectableTwo'. Скрипт отключён.");
            enabled = false;
            return;
        }
    ...
    void OnMouseDown()
    {
        ...
        Debug.Log($"OnMouseDown: {gameObject.name} - hasTouchedEnemyFloor: {hasTouchedEnemyFloor}, hasTouchedTeamFloor: {hasTouchedTeamFloor}");
    ...
    public void Select()
    {
        ...
        if (hasTouchedTeamFloor && !hasTouchedEnemyFloor)
        {
            ...
            // Выключить движение/перенос у всех остальных объектов
            foreach (PlayerSelectionTwo obj in Object.FindObjectsByType<PlayerSelectionTwo>(FindObjectsSortMode.None))
            {
                if (obj != this)
                {
                    obj.Deselect();
        ...
        if (hasTouchedEnemyFloor && !hasTouchedTeamFloor)
        {
            ...
            // Выключить движение/перенос у всех остальныех объектов
            foreach (PlayerSelectionTwo obj in Object.FindObjectsByType<PlayerSelectionTwo>(FindObjectsSortMode.None))
            {
                if (obj != this)
                {
        ...
        if (!hasTouchedTeamFloor && !hasTouchedEnemyFloor)
        {
            Debug.LogError("Игрок ничего не пересекает (PlayerSelectionTwo)");
        }
